...
def atc_search_view(request):
    ...
    if request.method == 'GET':
                ...
                if len(atc_code_inp) > 5:  # search for id length =7
                    ...
                    if len(atc_code_inp) == 5:  # search for id length =5
                        ...
                        if len(atc_code_inp) == 4:  # search for id length =5
                            results += list(
                                AtcPharmacologicalGroup.objects.filter(id__iexact=atc_code_inp).values('id', 'name'))
                        else:
                            if len(atc_code_inp) == 3:  # search for id length =5
                                results += list(
                                    AtcTherapeuticGroup.objects.filter(id__iexact=atc_code_inp).values('id', 'name'))
                            else:
                                results += list(
                                    AtcAnatomicalGroup.objects.filter(id__iexact=atc_code_inp).values('id', 'name'))
        if atc_name_inp != "":
            if query_option == 'containing':
                ...
                results += list(
                    AtcPharmacologicalGroup.objects.filter(name__icontains=atc_name_inp).values('id', 'name'))
                ...
                results += list(
                    AtcPharmacologicalGroup.objects.filter(name__startswith=atc_name_inp).values('id', 'name'))
        ...
        context = {"query_option": query_option, "search_result": results, "atc_code_inp": atc_code_inp,
...
def get_drug_atc_association(request):
    ...
    associations_list = [
        {"drug_bankID": assoc.drug_id.drug_bankID, "name": assoc.drug_id.name, "description": assoc.drug_id.description}
...
def get_drug_network_frame(request):
    drug_bank_id = request.GET.get("drugbank_id");

    if not drug_bank_id:
        return Http404("No drugbank_id provided.")
